...
class CrossModel(BertPreTrainedModel):
    def __init__(self, config, cls_loss=True, graph=False, layer=1, data_path=None,
        ...
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.classifiers = nn.ModuleList([nn.Linear(self.bert.config.hidden_size, num_labels) for num_labels in num_labels_list])
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if self.training:
                cross_attention_mask = self.graph_encoder(outputs['inputs_embeds'],
                                                   attention_mask, labels, lambda x: self.bert.embeddings(x)[0])

                cross_attention_output = self.bert(
                    input_ids,
                    attention_mask=attention_mask,
                    token_type_ids=token_type_ids,
                    position_ids=position_ids,
                    ...
                    output_hidden_states=output_hidden_states,
                    return_dict=return_dict,
                    embedding_weight=cross_attention_mask,
                )
                cross_attention_sequence_output = self.dropout(self.pooler(cross_attention_output[0]))
                logits = []
                for classifier in self.classifiers:
                    logits.append(classifier(cross_attention_sequence_output))
        ...
        return {
            ...
            'num_labels_list': self.num_labels_list,
            'labels': labels
        }
