...
parser.add_argument('--nsl', default=0.01, type=float, help='Weight of negative supervision.')
parser.add_argument('--gama_neg', default=3, type=float, help='gamma_neg of ASL.')
parser.add_argument('--gama_pos', default=1, type=float, help='gamma_neg of ASL.')
...
if __name__ == '__main__':
    ...
    lora_config = LoraConfig(
        r=128,
        lora_alpha=4096,
        target_modules=['bert.encoder.layer.0.intermediate.dense','bert.encoder.layer.0.output.dense','bert.encoder.layer.1.intermediate.dense','bert.encoder.layer.1.output.dense','bert.encoder.layer.2.intermediate.dense','bert.encoder.layer.2.output.dense','bert.encoder.layer.3.intermediate.dense','bert.encoder.layer.3.output.dense','bert.encoder.layer.4.intermediate.dense','bert.encoder.layer.4.output.dense','bert.encoder.layer.5.intermediate.dense','bert.encoder.layer.5.output.dense','bert.encoder.layer.6.intermediate.dense','bert.encoder.layer.6.output.dense',
    'bert.encoder.layer.7.intermediate.dense','bert.encoder.layer.7.output.dense','bert.encoder.layer.8.intermediate.dense','bert.encoder.layer.8.output.dense','bert.encoder.layer.9.intermediate.dense','bert.encoder.layer.9.output.dense','bert.encoder.layer.10.intermediate.dense','bert.encoder.layer.10.output.dense','bert.encoder.layer.11.intermediate.dense','bert.encoder.layer.11.output.dense'],
        lora_dropout=0.05,
        bias="none",
    ...
    for i in range(experts):  # 创建三个模型，可以根据需要更改数量
        model = ContrastModel.from_pretrained('bert-base-uncased', num_labels=num_class,
                                              contrast_loss=args.contrast, graph=args.graph,
                                              layer=args.layer, data_path=data_path, multi_label=args.multi,
                                              lamb=args.lamb, threshold=args.thre, tau=args.tau, name=args.name,
                                              NSL=args.nsl, gama_neg=args.gama_neg, gama_pos=args.gama_pos).to(device)
        if i == 0:
            # model.bert_embeddings = shared_embeddings
    ...
    # 遍历每一折
    for fold in range(n_splits):  # 只循环前四折
        # 计算当前折应有的数据量
    ...
    # model, optimizer, train= accelerator.prepare(model, optimizer, train)
